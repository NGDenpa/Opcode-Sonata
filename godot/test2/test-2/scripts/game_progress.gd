...
static var max_completed_level: int = -1
...
static func complete_level(level_idx: int) -> void:
	max_completed_level = maxi(max_completed_level, level_idx)
	unlock(level_idx + 1)
...
static func is_music_unlocked(level_idx: int) -> bool:
	if is_debug_enabled():
		return true
	return level_idx <= max_completed_level
